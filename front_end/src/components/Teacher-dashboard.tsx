...
const [selectedClass, setSelectedClass] = useState<string>("10");
 const [teacherClass, setTeacherClass] = useState<string>("");
  const [teacherSection, setTeacherSection] = useState<string>("");
  useEffect(() => {
    ...
    // Fetch teacher info
   axios
  .get<Teacher>(`http://localhost:3000/login/getTeacher/${teacherUsername}`)
  .then((res) => {
    setTeacherClass(res.data.class);
    setTeacherSection(res.data.section); // ADD THIS
  })
...
{/*  */}
    ...
    <div style={{ marginTop: "10px" }} className="pd-right">
      ...
      <hr style={{ marginBottom: "23px",marginTop: "10px",opacity:"0.5",marginLeft:'45px',marginRight:"45px" }}></hr>
