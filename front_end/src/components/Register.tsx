...
if (errorInputs.length > 0) {
  ...
  phones
    ...
    try {
      const res = await axios.post('http://localhost:3000/login/postStudent', userData);
...
if (!/^[a-zA-Z ]{3,}$/.test(formValues.username.trim())) {
  errors.username = 'Name must be at least 3 characters and contain only letters.';
  isValid = false;
}
...
{formErrors.roll && <p className="p19">{formErrors.roll}</p>}
  ...
  <p className="p26">{formErrors.english}</p>
        ...
        <div className='div3'>
          <button className='p17' type="submit">+ Add</button>
        </div>
        </form>
      </div>
    </div>
